import { dbGetAllAlliance, dbInsertAlliance, dbUpdateAlliance, dbGetOneAlliance, dbDeleteAlliance } from '../basededatos/allianceDB.js'

const srvGetAllAlliance = async () => {
    const jsonAllAlliance = { 'statusSrvc': '', 'responseDB': '0' }
...
const srvGetOneAlliance = async (idAlliance) => {
    const respOneAlliance = { 'statusSrvc': '', 'responseDB': '0' }
...
const srvCreateNewAlliance = async (newAlliance) => {
    const respNewAlliance = { 'statusSrvc': '', 'responseDB': '0' }
    ...
    if (allianceExist.status === "error") {
        ...
        if (allianceAdded.data.length > 0) {
...
const srvUpdateOneAlliance = async (uptAlliance) => {
    const respUpdAlliance = { 'statusSrvc': '', 'responseDB': '0' }
...
const srvDeleteOneAlliance = async (delAlliance) => {
    const respDelAlliance = { 'statusSrvc': '', 'responseDB': '0' }
